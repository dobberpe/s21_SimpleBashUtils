...
grep_args* get_options(int argc, char** argv) {
    ...
    while ((option = getopt(argc, argv, "AbeEnstT")) != -1 && !fail) {
        if (option == 'e') grep->regex = true;
        ...
        else if (option == 'f') grep->fregex = true;
        else if (option == 'o') grep->part = true;
        else fail = true;
    }

    if (grep->num_nonblank && grep->num) grep->num = false;
